...
package org.audiofx.mp3
{
	...
	internal class MP3Parser extends EventDispatcher
	{
		...
		public var constantFrameSize:uint;
		...
		private function getFirstHeaderPosition():uint //获得第一帧音频数据，即跳过id3
		{
			mp3Data.position=0;
			...
			while(mp3Data.position<mp3Data.length)
			{
				...
				//trace("getfirstheader working");
				if(isValidHeader(val))
				{
					parseHeader(val);
					constantFrameSize=getFrameSize(val);
					
					frameTime=(constantFrameSize*8)/BitRate;
					mp3Data.position=readPosition+constantFrameSize;
					trace("BitRate"+BitRate);
					trace("frametime "+frameTime);
					if(isValidHeader(mp3Data.readInt()))
					{
						//trace("id3"+readPosition);
						//trace("every frame of time"+frameTime);
						//trace("bitRate"+BitRate);
		...
		private function getCRC(headerBits:uint):uint
		{
			return uint((headerBits>>16)&1);}
	}
}