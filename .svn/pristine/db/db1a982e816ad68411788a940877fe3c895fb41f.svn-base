...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        hasChildren: function (treenodedata)
        {
            ...
            return treenodedata[this.options.childrenFieldName] ? true : false;
        },
        //获取父节点 数据
        ...
        arrayToTree: function (data, id, pid)      //将ID、ParentID这种数据格式转换为树格式
        {
            ...
            for (var i = 0; i < itemLength; i++)
            {
                ...
                parentData[this.options.childrenFieldName] = parentData[this.options.childrenFieldName] || [];
                parentData[this.options.childrenFieldName].push(currentData);
            }
            return targetData;
        },
        //根据数据索引获取数据
        _getDataNodeByTreeDataIndex: function (data, treedataindex)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                if (data[i].treedataindex == treedataindex)
                    return data[i];
                if (data[i][p.childrenFieldName]) {
                    var targetData = g._getDataNodeByTreeDataIndex(data[i][p.childrenFieldName], treedataindex);
        ...
        _setTreeDataStatus: function (data, status)
        {
            ...
            $(data).each(function ()
            {
                ...
                if (this[p.childrenFieldName]) {
                    g._setTreeDataStatus(this[p.childrenFieldName], status);
                }
            });
        },
        //设置data 索引
        _addTreeDataIndexToData: function (data)
        {
            ...
            $(data).each(function ()
            {
                ...
                if (this[p.childrenFieldName]) {
                    g._addTreeDataIndexToData(this[p.childrenFieldName]);
                }
            });
        },
        _addToNodes: function (data)
        {
            ...
            g.nodes = g.nodes || []; 
            g.nodes.push(data);
            if (!data[p.childrenFieldName]) return;
            $(data[p.childrenFieldName]).each(function (i, item)
            {
                g._addToNodes(item);
            });
        },
        ...
        _appendData: function (treeNode, data)
        {
            ...
            if (treenodedata && treenodedata[p.childrenFieldName] == undefined) treenodedata[p.childrenFieldName] = [];
            $(data).each(function (i, item) {
                if (treenodedata)
                    treenodedata[p.childrenFieldName][treenodedata[p.childrenFieldName].length] = item;
                else
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                if (g.hasChildren(o))
                {
                    ...
                    { 
                        if (delay == true)
                        {
                            g.toggleNodeCallbacks.push({
                                ...
                                {
                                    var content = g._getTreeHTMLByData(o[p.childrenFieldName], outlineLevel + 1, isLastNew, isExpandCurrent).join('');
                                    $(dom).append(content);
                    ...
                    {
                        treehtmlarr.push(g._getTreeHTMLByData(o[p.childrenFieldName], outlineLevel + 1, isLastNew, isExpandCurrent).join(''));
                    }
                }
                treehtmlarr.push('</li>');
            }
        ...
        _getDataByTreeHTML: function (treeDom)
        {
            ...
            $("> li", treeDom).each(function (i, item)
            {
                ...
                if ($("> ul", this).length > 0)
                {
                    data[dataindex][p.childrenFieldName] = g._getDataByTreeHTML($("> ul", this));
                }
            });
            return data;
        },
            ...
            if ($.fn.ligerDrag && p.nodeDraggable)
            {
                ...
                g.tree.ligerDrag({
                    ...
                    onRendered: function ()
                    {
                        ...
                        g[p.childrenFieldName][this.id] = this;
                    },
                    ...
                    onStopDrag: function (current, e)
                    {
                        ...
                        if (g.nodeDropIn != -1)
                        {
                            ...
                            {
                                var children = nodes[i][p.childrenFieldName];
                                if (children) {
